...
colorama cryptography dateutil dbus dev hidapi idna libgpiod marshmallow more-itertools
multidict netifaces packaging passlib pillow ply psutil pycparser pyelftools pyghmi
pygments pyparsing requests semantic-version setproctitle six spidev systemd tabulate
urllib3 wrapt xlib yaml yarl" )
  do
    pkgs="$pkgs python3-$i"
  done
  ...
  # U
  pip3 install dbus_next==0.2.3 zstandard==0.19.0 pyserial==3.5 aiohttp==3.8.3 setuptools==58.2.0
...
get-packages() { 
  ...
  if [ $CUSTOM_KVMD_VERSION -eq 1 ]; then
    ...
    PIKVM_PKGS=$( pikvm_pkgs_custom )
    else
        PIKVM_PKGS=$( pikvm_pkgs )
  fi
...
# then uncompress, kvmd-{version}, kvmd-webterm, and janus packages 
  ...
  # Use custom kvmd version replace kvmd offical package
    ...
    #tar -vxf "${KVMDCACHE}/kvmd-common.tar.gz" -C /
...
install-dependencies() {
  ...
  $APT_EXE install -y nginx python3 bc expect v4l-utils gpiod dialog git python3-pip tesseract-ocr tesseract-ocr-eng tesseract-ocr-chi-sim jq
  install-python-packages libxkbcommon-x11-0
...
pikvm_pkgs() {
    egrep 'janus|kvmd' ${PKGINFO} | grep -v sig | cut -d'>' -f1 |\
        cut -d'"' -f2 | egrep -v 'fan|oled' |\
        egrep 'janus|pi4|webterm|kvmd-[0-9]'
} # end pikvm_pkgs

pikvm_pkgs_custom() {
    egrep 'janus|kvmd' ${PKGINFO} | grep -v sig | cut -d'>' -f1 |\
        cut -d'"' -f2 | egrep -v 'fan|oled' |\
        egrep 'janus|pi4|webterm|kvmd-[0-9]' | egrep -v 'kvmd-[0-9]'
} # end pikvm_pkgs_custom
